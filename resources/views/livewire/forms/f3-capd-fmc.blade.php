...
<div class="p-4 max-w-5xl mx-auto" x-data="markCounter(@js($answers))">
  ...
  <h1 class="text-3xl font-bold text-gray-900">
    CAPD Exchange Assessment Form
    <br>
    <small class="text-xl text-gray-600 block mt-1">(Patient/Caregiver)</small>
  </h1>
  <h3 class="text-2xl text-[#2c3e50] font-semibold mt-4">FMC System</h3>
  <p class="text-lg text-gray-500 mt-1">
    System: <strong>CAPD Fresenius (Stay Safe)</strong>
  </p>
</div>
...
<div class="grid grid-cols-1 md:grid-cols-3 gap-4 mb-4">
    ...
    <!-- Tailwind-styled select box -->
    <div class="flex flex-col col-span-1 md:col-span-1">
        <label for="dialysis_centre" class="mb-1 text-sm font-medium text-gray-700">Current Dialysis Centre</label>
       <select wire:model="dialysis_centre" name="dialysis_centre" id="dialysis_centre"
    class="border border-gray-300 rounded-md px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-blue-500 disabled:bg-gray-100"
    {{ $isSubmitted ? 'disabled' : '' }}>
    <option value="">-- Select NKF Dialysis Centre --</option>
    <option value="nkf_klang">NKF Klang</option>
    <option value="nkf_setapak">NKF Setapak</option>
    <option value="nkf_cheras">NKF Cheras</option>
    <option value="nkf_subang">NKF Subang</option>
    <option value="nkf_ipoh">NKF Ipoh</option>
    <option value="nkf_penang">NKF Penang</option>
    <option value="nkf_johor_bahru">NKF Johor Bahru</option>
    <option value="nkf_kota_bharu">NKF Kota Bharu</option>
    <option value="nkf_kuching">NKF Kuching</option>
    <option value="nkf_kota_kinabalu">NKF Kota Kinabalu</option>
</select>
@error('dialysis_centre') <span class="text-red-500 text-sm">{{ $message }}</span> @enderror

    </div>

    <x-form-input label="Assessment done by" model="assessment" :disabled="$isSubmitted"/>
